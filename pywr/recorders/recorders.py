...
class HydroPowerRecorder(NodeRecorder):
    ...
    energy_unit_conversion : float (default=1e-6)
        A factor used to transform the units of total energy. Defaults to 1e-6 to return $MJ$. For example
        set this to 2.7778e-7 (1 / 3,600,000) to convert to $kWh$.

    Notes
    -----

    The flow units must be converted to $m^3$ per day (not per second). 

    Head is calculated from the given Storage node and/or level as follows:
     ...
     - If only level is given then this is simply used as a constant head.
    ...
    def after(self):
        ...
        for scenario_index in self.model.scenarios.combinations:
            ...
            self._data[ts.index, scenario_index.global_id] = power * delta * self.energy_unit_conversion
