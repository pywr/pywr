...
class TestDownSampling:
    ...
    @pytest.mark.parametrize('down_sample_func', ['mean', 'max'])
    def test_daily_to_monthly(self, down_sample_func):
        """Test daily data to monthly model time-step."""
        input_df = make_df('D')
        model_index = make_model_index('M')

        input_resampled = align_and_resample_dataframe(input_df, model_index, down_sample_func=down_sample_func)

        pd.testing.assert_index_equal(input_resampled.index, model_index)
        pd.testing.assert_frame_equal(input_resampled, input_resampled.resample('M').agg(down_sample_func))

    @pytest.mark.parametrize('down_sample_func', ['mean', 'max'])
    def test_daily_to_weekly(self, down_sample_func):
        ...
        pd.testing.assert_index_equal(input_resampled.index, model_index)
        pd.testing.assert_frame_equal(input_resampled, input_resampled.resample('W').agg(down_sample_func))

    @pytest.mark.parametrize('down_sample_func', ['mean', 'max'])
    def test_daily_to_7daily(self, down_sample_func):
        ...
        pd.testing.assert_index_equal(input_resampled.index, model_index)
        pd.testing.assert_frame_equal(input_resampled, input_resampled.resample('7D').agg(down_sample_func))

    @pytest.mark.parametrize('down_sample_func', ['mean', 'max'])
    def test_misaligned_daily_to_7daily(self, down_sample_func):
        """Test daily to 7-day model time-step."""
    ...
    @pytest.mark.parametrize('down_sample_func', ['mean', 'max'])
    def test_weekly_to_monthly(self, down_sample_func):
    ...
    @pytest.mark.parametrize('down_sample_func', ['mean', 'max'])
    def test_weekly_to_two_weekly(self, down_sample_func):
        ...
        pd.testing.assert_index_equal(input_resampled.index, model_index)
        pd.testing.assert_frame_equal(input_resampled, input_resampled.resample('2W').agg(down_sample_func))
...
class TestUpSampling:
    ...
    @pytest.mark.parametrize('down_sample_func', ['mean', 'max'])
    def test_weekly_to_7daily(self, down_sample_func):
        ...
        expected_df = input_df.resample('D').ffill()
        expected_df = expected_df['2015-01-01':'2015-12-31']
        expected_df = expected_df.resample('7D').agg(down_sample_func)

        pd.testing.assert_index_equal(input_resampled.index, model_index)
        pd.testing.assert_frame_equal(input_resampled, expected_df)

    @pytest.mark.parametrize('down_sample_func', ['mean', 'max'])
    def test_nonaligned_weekly_to_7daily(self, down_sample_func):
