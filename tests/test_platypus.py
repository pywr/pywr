...
class TestPywrRandomGenerator:
    ...
    def test_other_solutions(self, two_reservoir_problem, use_current):
        ...
        solutions = [
            ...
            {"control_curve": {"doubles": [-2] * 6 + [2] * 6}},  # This solution is out of bounds
        ]
        ...
        if use_current:
            ...
            np.testing.assert_allclose(algorithm.population[1].variables, np.ones(12) * 0.5)
            np.testing.assert_allclose(
                algorithm.population[2].variables, np.r_[np.zeros(6), np.ones(6)]
            )
        else:
            np.testing.assert_allclose(algorithm.population[0].variables, np.ones(12) * 0.5)
            np.testing.assert_allclose(
