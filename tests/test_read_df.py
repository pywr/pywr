...
def dataframe_for_test():
    return pd.DataFrame({"a": [1, 2, 3]}, index=pd.date_range("2020-01-01", periods=3, freq="D"))
...
def test_read_dataframe_rds(tmp_path):
    ...
    df = pd.DataFrame({"a": [1, 2, 3]}, index=pd.date_range("2020-01-01", periods=3, freq="D"))
    ...
    result = read_dataframe(model, data.copy())
    assert np.allclose(result["a"], df["a"])
    assert isinstance(result.index, pd.DatetimeIndex) or isinstance(result.index, pd.Index)